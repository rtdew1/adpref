...
def compute_gp_var(new_x, x, hypers):
    noise = hypers["noise"]
    K_newx_x = K(new_x, x, hypers)
    K_sigma = K(x, x, hypers) + np.diag((noise**2) * np.ones(x.shape[0]))
    K_coef = K_newx_x @ np.linalg.inv(K_sigma)
    return K_self_diag(new_x, hypers) - diag_dot_product(K_coef, K_newx_x.T)
...
def nlmap(pars, args):
    ...
    # log prior for amplitude: default is amplitude ~ inv-gamma(12,3)
    if "amp_prior" in args.keys():
        lp_amp = invgamma_lpdf(
            amp, a=args["amp_prior"]["a"], b=args["amp_prior"]["scale"]
        )
    else:
        lp_amp = invgamma_lpdf(amp, a=1.5, b=3)

    return -(lml + lp_noise + lp_ls + lp_amp)
