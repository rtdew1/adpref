...
class PrefOptim(PrefOptimBase):
    def __init__(
        ...
        map=True,
        init=[0.1, 0.4, 0.8],
        addl_optim_args=None,
    ):
        super().__init__()
        print("Backend: Using OG model")
    ...
    def ucb(self, z, scaling_factor):
        gp_mean, gp_cov = gp_inf.compute_gp(
            new_x=z, x=self.x, y=self.y, hypers=self.hypers
        )
    ...
    def update_posterior(self, new_rating, new_z, update_hypers=False):
        print("Updating posterior: new_z = ", new_z, ", new_rating = ", new_rating)
    ...
    def ends_training(self):
        ...
        print("Renormalizing ratings and fixing amplitude for test phase.")
