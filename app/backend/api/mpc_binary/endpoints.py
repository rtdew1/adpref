...
from api.mpc_binary.responses import move_on, show_image, do_arbitrary_check
from api.mpc_binary.util import rescale_ratings
from api.shared.responses import completed, reply_mturk_id
from data_io.mpc_binary.dataset import all_items, z_all
...
def post_refine_receive(user_id: uuid.UUID):
    ...
    if result == "none" and num_done_ratings < num_items_shown:
        ...
        update_hypers = (
            ((num_done_ratings > 0) and (num_done_ratings % UPDATE_HYPERS_ITERS == 0)) or num_done_ratings == user_config["NUM_REFINE_RATINGS"]
        )
