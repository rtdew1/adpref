...
from api.ar.util import rescale_ratings
from api.ar.responses import move_on, show_image, do_arbitrary_check
from api.shared.responses import completed, reply_mturk_id
from data_io.ar.dataset import all_items, z_all
from data_io.shared.user import (
    get_user_config,
    get_user_data,
    save_user_data,
...
def post_build_model(user_id: uuid.UUID):
    ...
    user_model = PrefOptim(np.array(z_shown), np.array(ratings), map=USE_MAP)
...
def get_refine_send(user_id: uuid.UUID):
    ...
    if num_displayed_items > num_done_ratings:  # An item was displayed but not rated
        ...
        item_id = first_element_not_in(
            sorted_next_items,
            user_data["initial_items_shown"],
            user_data["refine_items_shown"],
        )
...
def post_refine_receive(user_id: uuid.UUID):
    ...
    print(f"current value of hypers: {user_data['pref_model'].hypers}")
    save_user_data(user_id, user_data)
    return completed()
...
def post_calc_results(user_id: uuid.UUID):
    ...
    learning_history = {
        ...
        "history_dict": str(
            {
                **{
                    user_data["initial_items_shown"][i]: (
                        initial_model_fit[i],
                        user_data["initial_ratings"][i],
                    )
                    for i in range(user_config["NUM_INITIAL_RATINGS"])
                },
                **{
                    user_data["refine_items_shown"][i]: (
                        refine_model_fit[i],
                        user_data["refine_ratings"][i],
                    )
    ...
    print(f"before saving learning, current value of hypers: {user_data['pref_model'].hypers}")
    save_learning_history(user_id, learning_history)
    print(f"after saving learning, current value of hypers: {user_data['pref_model'].hypers}")
    ordered_items = np.argsort(predicted_utility_all)

    unseen_items_sorted_best_first = np.setdiff1d(
        ordered_items[::-1],
        user_data["initial_items_shown"] + user_data["refine_items_shown"],
        assume_unique=True,
    ).tolist()
    ...
    save_user_data(user_id, user_data)
    print(f"after generating test set, current value of hypers: {user_data['pref_model'].hypers}")
    return completed()
...
def post_test_receive(user_id: uuid.UUID):
    ...
    print(f"after receiving test rating, current value of hypers: {user_data['pref_model'].hypers}")
    save_user_data(user_id, user_data)
    return completed()
...
def get_load_recs(user_id: uuid.UUID):
    ...
    save_user_data(user_id, user_data)
    print(f"after saving test history, current value of hypers: {user_data['pref_model'].hypers}")
